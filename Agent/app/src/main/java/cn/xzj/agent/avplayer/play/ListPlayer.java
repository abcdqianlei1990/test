...
import cn.xzj.agent.avplayer.base.BSPlayer;
...
public class ListPlayer extends BSPlayer {
    ...
    protected RelationAssist onCreateRelationAssist() {
        RelationAssist assist = new RelationAssist(MyApplication.application.getApplicationContext());
        assist.setEventAssistHandler(new OnAssistPlayEventHandler(){
            @Override
            public void requestRetry(AssistPlay assistPlay, Bundle bundle) {
                if(DisplayUtil.isTopActivity(mActivityRefer!=null?mActivityRefer.get():null)){
