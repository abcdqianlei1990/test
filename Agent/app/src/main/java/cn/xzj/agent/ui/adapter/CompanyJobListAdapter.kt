 ...
 * @Des 岗位列表adapter
 */
class CompanyJobListAdapter : QuickAdapter<JobInfo>(R.layout.item_company_job) {
    override fun convert(holder: BaseHolder, item: JobInfo, position: Int) {
        ...
        if (item.recruitmentNeeds.isNotEmpty()) {
            ...
            if (!TextUtils.isEmpty(recuimentNeedInfo.interviewDate)) {
                ...
                holder.getView<View>(R.id.item_job_list_interviewdate_tv).visibility=View.VISIBLE
            }else{
                holder.getView<View>(R.id.item_job_list_interviewdate_tv).visibility=View.GONE
            }
            //收费金额
            ...
            holder.getView<View>(R.id.item_job_list_reward_tv).visibility=View.GONE
            holder.getView<View>(R.id.item_job_list_interviewdate_tv).visibility=View.GONE
            holder.getView<View>(R.id.item_job_list_toll_tv).visibility=View.GONE
        }
        ...
        //小职姐提成按钮
        var conditionTv = holder.getView<AppCompatTextView>(R.id.item_job_list_rewardCondition_tv)
        var conditionBtn = holder.getView<AppCompatTextView>(R.id.item_job_list_rewardCondition_btn)
        conditionTv.setOnClickListener {
    ...
    private var listener: OnItemClickListener<JobInfo>? = null
    fun setOnRewardConditionBtnClickListener(listener: OnItemClickListener<JobInfo>){
