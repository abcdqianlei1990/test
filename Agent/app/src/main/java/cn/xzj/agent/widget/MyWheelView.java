...
public class MyWheelView extends View {
    ...
    private MyWheelViewMessageHandler handler;
    ...
    private void initLoopView(Context context) {
        ...
        handler = new MyWheelViewMessageHandler(this);
        gestureDetector = new GestureDetector(context, new MyLoopViewGestureListener(this));
    ...
    public void smoothScroll(WheelView.ACTION action) {//平滑滚动的实现
        ...
        mFuture = mExecutor.scheduleWithFixedDelay(new MySmoothScrollTimerTask(this, mOffset), 0, 10, TimeUnit.MILLISECONDS);
    }

    public final void scrollBy(float velocityY) {//滚动惯性的实现
        cancelFuture();
        mFuture = mExecutor.scheduleWithFixedDelay(new MyInertiaTimerTask(this, velocityY), 0, VELOCITY_FLING, TimeUnit.MILLISECONDS);
    }
            ...
            default:
                if (!eventConsumed) {//未消费掉事件
                    ...
                    if ((System.currentTimeMillis() - startTime) > 120) {
                        // 处理拖拽事件
                        smoothScroll(WheelView.ACTION.DAGGLE);
                    } else {
                        // 处理条目点击事件
