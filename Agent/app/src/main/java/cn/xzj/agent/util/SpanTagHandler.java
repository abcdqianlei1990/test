...
public class SpanTagHandler implements Html.TagHandler {
    ...
    private final String TAG = "SpanTagHandler";
    ...
    public SpanTagHandler(Context context,ColorStateList originColors){
        mContext = context;
    ...
    public void endSpan(String tag, Editable output, XMLReader xmlReader){
        ...
        if (!TextUtils.isEmpty(size)) {
            int fontSizePx = 16;
            if (null != mContext){
                fontSizePx = UIUtil.dip2px(mContext,Integer.parseInt(size));
            }
    ...
    private void analysisStyle(int startIndex,int stopIndex,Editable editable,String style){
        ...
        if (!TextUtils.isEmpty(fontSize)) {
            int fontSizePx = 16;
            if (null != mContext){
                fontSizePx = UIUtil.dip2px(mContext,Integer.parseInt(fontSize));
            }
            editable.setSpan(new AbsoluteSizeSpan(fontSizePx), startIndex, stopIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        if (!TextUtils.isEmpty(fontWeight) && fontWeight.equalsIgnoreCase("bold")) {
            editable.setSpan(new StyleSpan(android.graphics.Typeface.BOLD), startIndex, stopIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
    }
}
