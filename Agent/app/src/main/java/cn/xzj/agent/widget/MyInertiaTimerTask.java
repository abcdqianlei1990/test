...
public class MyInertiaTimerTask extends TimerTask {
     ...
     */
    public MyInertiaTimerTask(MyWheelView wheelView, float velocityY) {
        super();
    ...
    public final void run() {
        ...
        if (Math.abs(mCurrentVelocityY) >= 0.0F && Math.abs(mCurrentVelocityY) <= 20F) {
            ...
            mWheelView.getHandler().sendEmptyMessage(MyWheelViewMessageHandler.WHAT_SMOOTH_SCROLL);
            return;
        }
        ...
        //刷新UI
        mWheelView.getHandler().sendEmptyMessage(MyWheelViewMessageHandler.WHAT_INVALIDATE_LOOP_VIEW);
    }
}
